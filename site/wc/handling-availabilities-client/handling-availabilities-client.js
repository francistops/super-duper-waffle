...
class handlingAvailabilitiesClient extends HTMLElement {
	...
	async connectedCallback() {
		...
		this.selectedDateDisplay = this.shadowRoot.getElementById(
			"selectedDateDisplay"
		);

		this.tbody = this.shadowRoot.querySelector(
		...
		inpDate.addEventListener("change", (e) => {
			...
			if (selected < min || selected > max) {
				...
				this.selectedDateDisplay.textContent = `Date sélectionnée : ${formatDate(
					this.selectedDate
				)}`;
			}
			this.tryRenderTable();
	...
	async renderTable(hairdresserId, selectedDate, serviceId) {
		...
		for (const a of filtered) {
			...
			const hairdresser = this.hairdressers.find(
				(h) => h.id === a.hairdresser_id
			);
			tdHairdresser.textContent = hairdresser
				? hairdresser.email
				: a.hairdresser_id;
